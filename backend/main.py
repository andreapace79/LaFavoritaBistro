...
def dev_seed(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            user = users_crud.create_user(db, users_schemas.UserCreate(username="admin", password="admin"))

        # 2) ruolo admin con wildcard
        role = rbac_crud.get_role_by_name(db, "admin")
        if not role:
            role = rbac_crud.create_role(
                db,
                rbac_schemas.RoleCreate(name="admin", description="Amministratore", permissions=["*"])
            )
        ...
        if not area:
            area = areas_crud.create_area(
                db, areas_schemas.AreaCreate(name="Interno", layout_meta={"note": "Area principale interna"})
            )
        ...
        return {
            ...
            "created_or_existing": {
                "user": user.username,
                "role": rbac_crud.get_role_by_name(db, "admin").name,  # garantito esista qui
                "area": area.name,
                "tables": ["T1", "T2"],
            },
        }
